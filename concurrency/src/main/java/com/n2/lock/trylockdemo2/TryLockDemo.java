package com.n2.lock.trylockdemo2;

import java.util.concurrent.locks.ReentrantLock;

class Shopper extends Thread {

  static int itemsInNotepad = 0; //shared notepad
  int itemsToAdd = 0;
  ...
  @Override
  public void run() {
    while (itemsInNotepad <= 20) { ////items present, add them to the notepad
      if ((itemsToAdd > 0) && (pencil.tryLock())) {
        ...
        try {
          Thread.sleep(100);//searching time
          itemsToAdd++;// collect
