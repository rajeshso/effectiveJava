package com.n2.challenges.downloadimage; /**
...
class ParallelImageDownloader {
  ...
  public int downloadAll() {
    ...
    List<Future<Integer>> futureList = new ArrayList<>(imageNumbers.length);
    for (int i = 0; i < imageNumbers.length; i++) {
      futureList.add(executorService.submit(new ParallelWorker(i)));
    }
    int totalBytes = 0;
    try {
  ...
  private class ParallelWorker implements Callable<Integer> {
    ...
    public ParallelWorker(int imageNumber) {
      this.imageNumber = imageNumber;
    }

    @Override
    public Integer call() throws Exception {
      return downloadImage(imageNumber);
    }
